...
import { SYSTEM_PROMPT_SEARCH, SYSTEM_PROMPT_NO_ANSWER_SUFFIX } from "../prompts/system.js";
...
export async function run(opts: RunOptions): Promise<SearchEnvelope> {
  ...
  const systemPrompt =
    SYSTEM_PROMPT_SEARCH + (opts.noAnswer ? SYSTEM_PROMPT_NO_ANSWER_SUFFIX : "");

  return callGrok({
    query: opts.query,
    model,
    systemPrompt,
    toolTypes: ["x_search", "web_search"],
